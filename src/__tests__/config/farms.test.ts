...
describe('Config farms', () => {
  ...
  it.each(farmsToTest)('Farm #%d has an unique address', (pid, farm) => {
    const duplicates = farms.filter((f) => farm.lpAddresses[4] === f.lpAddresses[4])
    expect(duplicates).toHaveLength(1)
  })

  it.each(farmsToTest)('Farm %d has the correct token addresses', async (pid, farm) => {
    const tokenAddress = farm.token.address[4]
    const quoteTokenAddress = farm.quoteToken.address[4]
    const lpContract = getLpContract(farm.lpAddresses[4])
  ...
  it.each(farmsToTest)('Farm %d has non 0 tokens amount', async (pid, farm) => {
    const tokenContract = getBep20Contract(farm.token.address[4])
    const quoteTokenContract = getBep20Contract(farm.quoteToken.address[4])

    const tokenAmount = await tokenContract.methods.balanceOf(farm.lpAddresses[4]).call()
    const quoteTokenAmount = await quoteTokenContract.methods.balanceOf(farm.lpAddresses[4]).call()
  ...
  const START_PID = 0
  const FACTORY_ADDRESS = '0x3611AD29A49ccb56A855eB5560078f835D76cDA4'
